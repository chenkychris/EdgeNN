...
int main(int argc, const char **argv) {
    ...
    cudaStreamAttachMemAsync(stream1, &c1_o, 0, cudaMemAttachHost);
    cudaStreamAttachMemAsync(stream1, &p1_a, 0, cudaMemAttachHost);
    ...
    cudaStreamAttachMemAsync(stream1, &c2_o, 0, cudaMemAttachHost);
    cudaStreamAttachMemAsync(stream1, &p2_a, 0, cudaMemAttachHost);
    ...
    cudaStreamAttachMemAsync(stream1, &c4_weight, 0, cudaMemAttachHost);
    cudaStreamAttachMemAsync(stream1, &c4_bias, 0, cudaMemAttachHost);
    cudaStreamAttachMemAsync(stream1, &c4_a, 0, cudaMemAttachHost);
    cudaStreamAttachMemAsync(stream1, &c4_z, 0, cudaMemAttachHost);
    cudaStreamAttachMemAsync(stream1, &c5_weight, 0, cudaMemAttachHost);
    cudaStreamAttachMemAsync(stream1, &c5_bias, 0, cudaMemAttachHost);
    cudaStreamAttachMemAsync(stream1, &c5_a, 0, cudaMemAttachHost);
    cudaStreamAttachMemAsync(stream1, &c5_z, 0, cudaMemAttachHost);
    cudaStreamAttachMemAsync(stream1, &p3_a, 0, cudaMemAttachHost);
    ...
    cudaStreamAttachMemAsync(stream1, &f3_weight, 0, cudaMemAttachHost);
    cudaStreamAttachMemAsync(stream1, &f3_bias, 0, cudaMemAttachHost);
    cudaStreamAttachMemAsync(stream1, &f3_a, 0, cudaMemAttachHost);
    cudaStreamAttachMemAsync(stream1, &f3_z, 0, cudaMemAttachHost);
...
static double forward_pass(double data[227][227][3], cudaStream_t stream1) {
    for (int i = 0; i < 227; ++i) {
        for (int j = 0; j < 227; ++j) {
            for (int k = 0; k < 3; k++) {
                input_a[i * 227 * 3 + j * 3 + k] = data[i][j][k];
            }
        }
    }
    ...
    if (offset != 1)
        fp_preact_c1_cpu((float(*)[227][3])L_input.output, (float(*)[55][55])L_c1.preact,
                         (float(*)[11][11][3])L_c1.weight, L_c1.bias);
    apply_step_function<<<112, 1280, 0, stream1>>>(L_c1.preact, L_c1.act_result, L_c1.O);
    ...
    apply_step_function<<<112, 1280, 0, stream1>>>(L_c2.preact, L_c2.act_result, L_c2.O);
    ...
    apply_step_function<<<128, 128, 0, stream1>>>(L_c3.preact, L_c3.output, L_c3.O);
    if (offset != 1)
        cudaEventRecord(c3);
    dim3 Bc4(12, 12);
    if (offset)
    ...
    apply_step_function<<<128, 128, 0, stream1>>>(L_c4.preact, L_c4.output, L_c4.O);
    if (offset != 1)
        cudaEventRecord(c4);
    ...
    apply_step_function<<<128, 128, 0, stream1>>>(L_c5.preact, L_c5.output, L_c5.O);
    dim3 ft_map2(6, 6);
    ...
    apply_step_function<<<128, 128, 0, stream1>>>(L_f1.preact, L_f1.output, L_f1.O);
    if (offset != 1)
        cudaEventRecord(f1);
    if (offset)
    ...
    apply_step_function<<<128, 128, 0, stream1>>>(L_f2.preact, L_f2.output, L_f2.O);
    if (offset != 1)
        cudaEventRecord(f2);
    if (offset)
    ...
    apply_step_function<<<128, 128, 0, stream1>>>(L_f3.preact, L_f3.output, L_f3.O);
